...
def main():
    ...
    try:
        t = fortigate_api('192.168.0.1', un, pw)
        #t.print_data (t.show(['cmdb', 'system', 'interface'], params={'action':'schema'}))
        ...
        #t.print_data (t.create(['cmdb', 'system', 'interface'], data={'name':'test_vlan', 'vlanid':3, 'type': 'vlan', 'ip': '1.1.1.1 255.255.255.0', 'interface':'port2', 'role':'lan', 'description':'test_vlan', 'asslowaccess':'ping', 'vdom':'root'}))
        #t.print_data (t.create(['cmdb', 'system', 'interface'], data={'name':'test_vlan', 'vlanid':3, 'type': 'vlan', 'ip': '1.1.1.1 255.255.255.0', 'interface':'port2', 'vdom':'root'}))
    except:
